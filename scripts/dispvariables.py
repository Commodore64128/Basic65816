...
class ListableVariableBlock(VariableBlock):
	#
	#		List variables.
	#
...
if __name__ == "__main__":
	...
	blk.addBASICLine('let demo = -2:demo2 = 42:demo3 = 99:a$="hi":bx1$="low":demo3 = demo+demo2:fred$ = "["+bx1$+"]"',10)
	#
	if False:
		for i in range(0,20):
			IntegerVariable().importVariable(blk)
			StringVariable().importVariable(blk)
			IntegerArray().importVariable(blk)
			StringArray().importVariable(blk)
	else:
		v1 = IntegerVariable("z",-260)
		print(v1.toString())
